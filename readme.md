...
There are optional parameters that you can enter in the command line (one or more of the above):
    ...
    myexe  [brickfile.txt]  spheres [spherefile.txt]  springs [springfile.txt]  motion_X [motionfilex.txt]   motion_Y [motionfiley.txt]    motion_Z [motionfilez.txt]  motion_dt  [dt]   motion_amplifier [a]  dt [dt]  T_max [t]  save_each [n]  snapshot_each [n] 
...
- *[yielding]* can be 0, 1 or 2. if the block is free, it doesn't matter. if the block is fixed, if 1, the block is absolutely fixed; if 0 and there is any motion file, the block follows the imposed motion law; if 2 and there is any motion file, an external force following the law in the motion file in applied to the block.
...
Example of a file with just two-blocks, one fixed, the other falling (two CR-terminated lines):
    ...
    0,1,1,1,1800, 0,0,0, 0.247655,3.048388,0.4, 0.458039,2.772294,0.0,   0.0,2.8,0.0,  0.0,3.3,0.0,  0.518308,3.3,0.0,  0.518308,3.268648,0.0,  0.458039,2.772294,0.8,  0.518308,3.268648,0.8,  0.0,2.8,0.8,  0.0,3.3,0.8,  0.518308,3.3,0.8
    1,0,1,1,1800, 0,0,0, 0.247655,4.048388,0.4, 0.458039,3.772294,0.0,0.0,3.8,0.0,0.0,4.3,0.0,0.518308,4.3,0.0,0.518308,4.268648,0.0,0.458039,3.772294,0.8,0.518308,4.268648,0.8,0.0,3.8,0.8,0.0,4.3,0.8,0.518308,4.3,0.8
...
just use a 9999 in the list of vertexes to separate the vertexes of the n-th sub-shape from the vertexes of the (n+1)-th shape, ex. 

    ... 0,1,2, 3,4,1,  9999  , 3,3,3, 4,2,3, ...
...
Note: springs resist to tension with the axial stiffness k, while they resist compression with an almost null value of stiffness (k->0).
...
- *Value* is the corresponding value of displecement in the motion law, in meters (in newton, if the motion file is used to apply forces). 
